...
Require Import List.
From Cyclic_PA.Casteran Require Import closure.
From Cyclic_PA.Casteran Require Import more_list.
Require Import Multiset.
From Cyclic_PA.Casteran Require Import list_permut.
...
Lemma Relation_equiv_lists : forall (R1 R2 : relation elt) (l1 l2 : list elt), RelationClasses.relation_equivalence R1 R2 -> multiset_extension_step R1 l1 l2 = multiset_extension_step R2 l1  l2.
Proof.
intros. induction l2.
- destruct l1.
  + assert (multiset_extension_step R1 nil nil). assert (forall a b, In b nil -> R1 b a). admit. pose (fun a => rmv_case (a:=a) (l:=nil) (l1:=nil) (l2:=nil) R1 nil (H0 a)). assert (exists (a : elt), a = a). admit. destruct H1. apply (m x). simpl. auto. simpl. admit.
Admitted.

Add Morphism multiset_extension_step : mult_morph.
Proof.
intros R1 R2 RF l1 l2 P12 l3 l4 P34. split; [intro R13 | intro R24].
apply list_permut_multiset_extension_step_2 with l3; trivial.
apply list_permut_multiset_extension_step_1 with l1; trivial. rewrite (Relation_equiv_lists (R2 := R1) l1 l3); auto.
unfold RelationClasses.relation_equivalence in*. unfold RelationClasses.predicate_equivalence in *. simpl. simpl in RF. intros. destruct (RF x x0) as [RF1 RF2]. split; auto. 
apply list_permut_multiset_extension_step_2 with l4; auto;
apply list_permut_multiset_extension_step_1 with l2; auto. rewrite (Relation_equiv_lists (R2 := R2) l2 l4); auto.
Qed.
...
Proof.
intros R1 R2 RF l1 l2 P; split; [intro A1 | intro A2].
apply list_permut_acc with l1; trivial; try setoid_rewrite <- P; auto.  admit.
apply list_permut_acc with l2; trivial. admit.
Admitted.
*)
