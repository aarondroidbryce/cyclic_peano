...
From Cyclic_PA.Casteran Require Import dickson.
Require Import Relations.
Require Import Wellfounded.
Require Import Arith.
Require Import Wf_nat.
From Cyclic_PA.Casteran Require Import term.
...
(*replace (size a + list_size size l) with (list_size size (a::l)).*)
rewrite <- eq1; trivial.
...
About length_map.
replace (length  ((fix map (l : list SN_term) : list term :=
             match l with
             | nil => nil (A:=term)
             | a :: t => tt a :: map t
             end) l))
with (length (map tt l)) in H6.
